...
<div class="row d-flex flex-row ">
  ...
  <div class=" sm-12 md-8 lg-8  py-2" id="infmed">
    <p>Esta área é destina-se ao registo dos sinais vitais dos utentes.</p>
    <p>Pode consultar os sinais vitais de cada utente, escolhendo o utente em cima.</p>
    <p>Para registar sinais vitais a um utente clica no botão +</p>
  </div>
</div>
...
$(document).ready(function() {
  $('button').click(function() {
    $('#dados2_').load('pesquisarutentesinais.php?id=' + $(this).attr('id') + '&estilo=' + $('#estil').val());
  });

  $('#nome').keyup(function() {
    $('#dados2_').load('pesquisarutentesinais.php?nome=' + $(this).val() + '&estilo=' + $('#estil').val());
  });
  ...
  $('#linha').click(function() {
    ...
    $('#estil').val('1')
    $('#dados2_').load('pesquisarutentesinais.php?nome=' + $(this).val() + '&estilo=1');
  });

  $('#coluna').click(function() {

    $('#estil').val('0')
    $('#dados2_').load('pesquisarutentesinais.php?nome=' + $(this).val() + '&estilo=0');
  });

  $('#novo').click(function() {
    $('#registar').val('1')
    $('#infmed').load('registasinais.php');
  });
  ...
  $('a#nomes').click(function() {
    ...
    if (ti == 1) {
      ...
      $('#infmed').load('sinal.php?id=' + $(this).attr('data'));
      $('html, body').animate({
        scrollTop: parseInt($("#infmed").offset().top - 100)
      }, 1000);
